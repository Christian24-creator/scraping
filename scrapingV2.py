...
st.set_page_config(
    page_title="Sufarmed - Buscador de Precios y Disponibilidad",
    page_icon="💊",
    layout="centered"
)

# Título principal
st.title("🏥 Sufarmed - Buscador de Precios y Disponibilidad")
st.markdown("---")
...
class SufarmedScraper:
    ...
    def login(self, email, password):
        ...
        try:
            ...
            success_indicators = [
                "mi-cuenta" in final_url, "my-account" in final_url, "dashboard" in final_url,
                "account" in final_url, "profile" in final_url, "bienvenido" in response_text,
                "welcome" in response_text, "logout" in response_text,
                "cerrar sesion" in response_text, "salir" in response_text
            ]
            
            error_indicators = [
                "error" in response_text and ("login" in response_text or "email" in response_text),
                "incorrect" in response_text, "invalid" in response_text and ("email" in response_text or "password" in response_text),
                "incorrecto" in response_text, "invalido" in response_text,
    ...
    def extract_products_from_html(self, html_content):
        """
        Extrae nombres, precios y disponibilidad de productos del HTML.
        """
        # Patrón del contenedor del producto. (Patrón del código original)
        product_container_pattern = r'<div[^>]*class=["\']product-miniature[^"\']*["\'][^>]*>(.*?)</div>'
        
        products = []
        product_blocks = re.findall(product_container_pattern, html_content, re.IGNORECASE | re.DOTALL)
        
        for block in product_blocks:
            # Patrón para el nombre del producto (Patrón del código original)
            ...
            # Patrón para el precio. (Patrón del código original)
            ...
            # 🟢 NUEVO: Lógica para detectar disponibilidad
            availability = "Agotado"
            if re.search(r'<span[^>]*class=["\']badge-success product-available', block, re.IGNORECASE):
    ...
    def buscar_producto(self, producto):
        """Busca un producto y obtiene una lista de resultados (nombre, precio, disponibilidad)"""
        try:
            search_urls = [
...
if st.button("🔍 Buscar Precio y Disponibilidad", type="primary"):
    if not email_input or not password_input:
            ...
            try:
                ...
                if resultados:
                    ...
                    for product in resultados:
                        ...
                        col_price, col_avail = st.columns(2)
                        with col_price:
                            st.metric(label="Precio", value=f"${product['price']}")
                        with col_avail:
                            color = "green" if product['availability'] == "Disponible" else "red"
                            st.markdown(f"<p style='color: {color}; font-weight: bold;'>Disponibilidad: {product['availability']}</p>", unsafe_allow_html=True)
                        st.markdown("---")
...
- **Paso 3**: Haz clic en "Buscar Precio y Disponibilidad"
- Esta aplicación busca precios y disponibilidad de productos en Sufarmed.com
- Utiliza `requests` y `regex` para extraer información
